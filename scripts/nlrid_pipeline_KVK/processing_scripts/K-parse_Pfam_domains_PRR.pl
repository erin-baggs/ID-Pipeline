...
while ($file=<PS>){                                                                                                
	...
	$outfile{$species}{'pk'} = $outdir . "/" . $basefile . ".pk.txt";
	$outfile{$species}{'pkid'} = $outdir . "/" . $basefile . ".pk-ID.txt";
...
#	    print Dumper($db{$species}{'Total_proteins'});
		...
		foreach (@domains){
			...
			if ($line=~ m/Pkinase/){ #check for the presence of NB-ARC domain                           

			    $nlr{$species}{$seqid}=$domainstring; #record all NLRs for a species 

			unless ($domainid=~ m/Pkinase|LRR|AAA|AAA/ ){ #skip known domains
...
open STATS1 , ">", $outdir . "/" . "pkid_domains-" . $today . ".stats.tsv";
...
open SUMMARYFILE, ">", $outdir . "/" . "pkrid_summary_table" . $today . ".tsv"; 

print SUMMARYFILE "Species", "\t", "NBS_count", "\t", "TIR-NBS_count", "\t", "NBS-fusion_count", "\t", "fusion_domains", "\n"; 

foreach my $speciesid (sort keys (%outfile) ){

    open FILEOUT1, ">", $outfile{$speciesid}{'pk'};
    open FILEOUT2, ">", $outfile{$speciesid}{'pkid'};
	...
	if ( $nlr_domains=~ m/LRR/ ){

	    $nlrtir_counter++;
	}
	
    }
    while ( my ($pkid_id, $pkid_domains) = each(%{$nlrsd{$speciesid}}) ){

	print FILEOUT2 $pkid_id, "\t", $pkid_domains, "\n";
	$nlrsd_counter++;
...
open SUMMARYFILE2, ">", $outdir . "/" . "pkid_by_prevalence" . $today . ".tsv";
open DATAFILE1, ">", $outdir . "/" . "pkid_by_prevalence_family_wordcloud_input" . $today . ".txt";
