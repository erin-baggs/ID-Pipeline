...
#		    unless ($domain=~ m/(/) die "Looks like pfamscan file does not contain verbose information for each domain\n"; 
			...
			if ($line=~ m/LRR.*+ Pkinase.*|Pkinase.*+LRR.*+/){ #check for the presence of Pkinase domain                           

			    $nlr{$species}{$seqid}=$domainstring; #record all Pkinases for a species 

			unless ($domainid=~ m/LRR.*+ Pkinase.*|Pkinase.*+LRR.*+/ ){ #skip known domains
