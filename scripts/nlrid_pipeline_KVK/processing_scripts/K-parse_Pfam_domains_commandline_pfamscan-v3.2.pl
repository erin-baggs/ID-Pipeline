...
# - v3.2 works with conventional pfamscan output rather than the one generated through pfam_scan.pl
...
my %evalue;
my $n=0;
#my ($query_id, $ali_from, $ali_to, $env_from, $env_to, $domain_id, $domain_name, $domain_type, $hmm_from, $hmm_to, $hmm_length, $bit_score, $evalue, $signif, $clan, $active_sites, $query_accesion, $query_length, $hash, $of, $c_Evalue, $i_Evalue, $score, $bias, $);
my ($domain_name, $domain_id, $hmm_length, $query_id, $query_accession, $query_length, $evalue, $bit_score, $bias1, $hash, $of, $c_Evalue, $i_Evalue, $score, $bias, $hmm_from, $hmm_to, $ali_from, $ali_to, $env_from, $env_to, $acc, $description);

my $tempfile1 = "./pfam.output.tmp";
my $tempfile2 = "./pfam.output.sorted.tmp";

open (FILETEMP, ">", $tempfile1);
...
system("sort -k 4,4 -k 18,18n $tempfile1 > $tempfile2");

open (FILEIN, "<", $tempfile2);
...
while ($line = <FILEIN> ){
    ...
    chomp $line; 
        ...
        #ChrUn.fgenesh.mRNA.8      89    182     83    182 PF03732.12  Retrotrans_gag    Family    13    96    96     49.3   3.6e-13   1 CL0523
        #($query_id, $ali_from, $ali_to, $env_from, $env_to, $domain_id, $domain_name, $domain_type, $hmm_from, $hmm_to, $hmm_length, $bit_score, $evalue, $signif, $clan, $active_sites) = split (/\t/, $li\


       #                                                                            --- full sequence --- -------------- this domain -------------   hmm coord   ali coord   env coord
       # target name        accession   tlen query name           accession   qlen   E-value  score  bias   #  of  c-Evalue  i-Evalue  score  bias  from    to  from    to  from    to  acc description of target
       #------------------- ---------- ----- -------------------- ---------- ----- --------- ------ ----- --- --- --------- --------- ------ ----- ----- ----- ----- ----- ----- ----- ---- ---------------------
       #NB-ARC               PF00931.17   287 TraesCS1A01G003100.1 -            787   3.3e-66  223.0   0.0   1   1   2.8e-69   4.6e-66  222.6   0.0     5   285   172   449   168   451 0.95 NB-ARC domain

	($domain_name, $domain_id, $hmm_length, $query_id, $query_accession, $query_length, $evalue, $bit_score, $bias1 , $hash, $of, $c_Evalue, $i_Evalue, $score, $bias, $hmm_from, $hmm_to, $ali_from, $ali_to, $env_from, $env_to, $acc, $description) = split (/\t/, $line);
