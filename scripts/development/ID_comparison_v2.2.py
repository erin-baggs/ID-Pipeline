...
def findAllDomains(fn, fn_2):
        ...
        for row in csv.reader(fi, delimiter='\t'): #changed from stdin to sys.argv
            if row:
                ...
                if domains:
                    ...
                    nlrids.add(row[0]) #List of domains which are in  NLR-IDs
        #return (id_names)
        #return (domain_info)
    #domains_info =findAllDomains(sys.argv[1])[0] ###HOW DO I RENAME THESE 
    #info_domains = findAllDomains(sys.argv[1])[1]

    #Now have list of domains - need to extract all proteins with these domains from parsed.verbose



    ##FUNCTION: Filter Domains matching ID domains for e-value threshold
    #def eValueFilter(fn, idList, domain_info): 
    with open(fn_2) as fi2:
        for row in csv.reader(fi2, delimiter='\t'): #changed from stdin to sys.argv
            if row:
                domains = list()
                for domain in row[1].split('~'):
                    name, coord = domain.split('(')
                    if name in id_names: # nlrids:
                        domains.append({'name': name})
                        #coord              start=162, stop=447, evalue=3.4e-75)
    ...
    print(domain_info)
    print(nlrids)
    return(domain_info, nlrids)
#eValueFilter(sys.argv[2], domain_names, info_domains)

##FUNCTION : Record only longest fasta transcript of ID matching domains

def longestFasta(fn, domain_info): 
    seen_id={}
    for _id, _seq in readFasta(fn, headless=True):            
        # print(_id)    
        _id=_id.split()[0]
        #counter=0
        gid ='initial'
        ...
        if _id in domain_info : #[1:] to remove > from fasta header in readFasta tool. sequences asks if its in dictionary general
            ...
            for domain in domain_info[_id]:
                ...
                if gid not in seen_id or len(_seq) > seen_id[gid][1]:
...
def domainExtract(fn, domain_info, seen_id): 
    print(domain_info)
    print(seen_id)
    with open('query.fa', 'w') as query_out, open('db.fa', 'w') as db_out : #DO I NEED TO ADD THESE FIELDS TO DEF
        files = dict()
        for _id, _seq in readFasta(fn, headless=True):
            _id = _id.split()[0]
            print('DE', _id)
            gid = ".".join(_id.split('.')[:-1])
            print('_id in domain_info: {}; gid in seen_id: {}; seen_id[gid][0] == _id: {}'.format(_id in domain_info, gid in seen_id, seen_id.get(gid, [None])[0] == _id), gid)
            if _id in domain_info and seen_id.get(gid, [None])[0] == _id : #[1:] to remove > from fasta header in readFasta tool. sequences asks if its in dictionary general
                print('DE2', _id)
...
def blastFilter(blast_db, query_fasta): ########NEED TO WORK ON THIS FUNCTION ###################
    blast_seen=set()
    alignment_domains=set()
    filtered_qid_sid=dict()
    with open('raw.blast', 'w') as blast_out, open('filtered.blast', 'w') as blast_filtered:

        stdout, stderr = makeBlastdb(blast_db)
        if not stderr :
            print ('working')
            for HSP in runBlast(query_fasta, BLAST_CMD_ALN, blast_db, 'blastp', nthreads=1): #default nthreads = 8  
    ...
    return (filtered_qid_sid, alignment_domains)


## FUNCTION : Print GFF co-ordinates of paralog to ID         
def GFFcoord(fn, filtered_qid_sid): 
    GFF_dict= {}
    with open(fn) as GFF, open('chromo_cord.gff', 'w') as chromo_coord:
...
def alignFasta(fn, fn_2, alignment_domains ):
    files_2=dict()
    ...
    for _id, _seq in readFasta(fn_2, headless=True):#Can I access newly created file this way? 
...
def alignment(files): ########### CHECK IF OK TO PUT list_files_2 outside of function 
    start_files=list()
    done_files=list()
    TCOFFEE_CMD = 'source t_coffee-9.03.r1318; touch {}; t_coffee {} -mode mcoffee -outfile {} -output fasta_aln; touch {};'
...
def aln_curation(alignment_files):
    seqs = dict((_id,list(_seq)) for _id,_seq in readFasta(alignment_files))
    #print(seqs)
...
def RAxML(files):
    ...
    RAXML_CMD = 'source lmod-6.1; ml raxml/8.2.9; touch {}; raxmlHPC-MPI-SSE3 -f a -x 1123 -p 2341 -# 100 -m PROTCATJTT -s {}  -n ID-RAXML; touch {};' #-n = output name #UNFINISHED
    ...
    start_time = time.time()
    unfinished = list(raxml_done_file)
    while True:
        unfinished = [f for f in unfinished if not os.path.exists(f)]
        if not unfinished or (time.time()-start_time)/60 > 120:
            print ('RAXML incomplete')
            print (unfinished)
            break
...
if __name__ == '__main__':
    ...
    domainInfo, nlrids = findAllDomains(sys.argv[1], sys.argv[2])
    print(nlrids)
    seenId = longestFasta(sys.argv[3], domainInfo)
    domainExtract(sys.argv[3], domainInfo, seenId)    
    filteredIds, alignmentDomains = blastFilter('db.fa', 'query.fa')##### NEED TO CHANG !!!! 
    GFFcoord(sys.argv[4], filteredIds)
    alignFasta('db.fa', 'query.fa', alignmentDomains)
    list_files_2 = glob.glob("*pre-aln.fa")
    alignment(list_files_2)
